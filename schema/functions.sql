CREATE OR REPLACE FUNCTION do_solve_attempt(att_user_id uuid, att_team_id uuid, att_challenge_id uuid, att_guess varchar(255)) RETURNS RECORD AS $$
    ...
    BEGIN
        ...
        IF guess_correct AND NOT already_solved THEN 
            PERFORM update_db_scores_solves();
        END IF;
...
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY user_data;
    REFRESH MATERIALIZED VIEW CONCURRENTLY team_data;
    REFRESH MATERIALIZED VIEW CONCURRENTLY challenge_data;
END;
$$ LANGUAGE plpgsql VOLATILE;
...
DROP TYPE IF EXISTS try_signin_ret CASCADE;
...
CREATE OR REPLACE FUNCTION get_top_n_teams(n integer) RETURNS SETOF uuid AS $$
    SELECT id FROM team_view ORDER BY score DESC, last_solve ASC, inserted_at ASC LIMIT n;
$$ LANGUAGE SQL STABLE;
